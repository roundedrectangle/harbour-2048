...
function addTiles (number, tiles, slots, size, componentTile, getValue, tilesCount) {
    ...
    for (var i = 0; i < number; i++) {
        ...
        if (tiles [index] === undefined) {
            tiles [index] = componentTile.createObject (slots [index], { "value" : getValue() });
...
function addTilesClassicEasy (number, tiles, slots, size, componentTile, tilesCount) {
    var getValue = function () { return 2; }
    addTiles(number, tiles, slots, size, componentTile, getValue, tilesCount);
}

function addTilesClassicNormal (number, tiles, slots, size, componentTile, tilesCount) {
    var getValue = function () { return Math.random() < 0.9 ? 2 : 4; }
    addTiles(number, tiles, slots, size, componentTile, getValue, tilesCount);
}

function addTilesClassicHard (number, tiles, slots, size, componentTile, tilesCount) {
...
function addTilesAdventureNormal (number, tiles, slots, size, componentTile, tilesCount) {
    var getValue = function () {
